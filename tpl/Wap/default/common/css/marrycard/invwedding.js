...
(function(window,$){
    ...
    var PATH_DATA = '/wedding/getinfo';
	//var PATH_DATA = 'http://e.weixiaoxin.cn/wedding/getinfo';
    //var PATH_DATA = 'http:///wedding/getinfo?callback=?';
    var PATH_JOINBLESS = '/wedding/joinbless';
    var DataGroup;
    var Directive = {
    ...
    $(function(){
        ...
        loadData(id,function(result){
            ...
            if(result['ret']==0 && result['data']){
                ...
                setForward(DataGroup);

                //3.预加载图片
                var imgs = [];
                if($.isArray(DataGroup['wedding']['photo'])){
                    $.each(DataGroup['wedding']['photo'],function(index,item){
                        imgs.push(item.src);
                    });
                }
                loadImages(imgs,function(){
                    renderTpl(DataGroup,Directive);
                    cover();
                });
    ...
    function getQueryString(name)
    {
        ...
        if (reg.test(location.href)) return decodeURIComponent(RegExp.$2.replace(/\+/g, " ")); return undefined;
    }
