...
class UpyunAction extends UserAction{
	...
	public function _initialize() {
	    ...
	    if (!empty($_FILES)) {
			...
			$this->redirect('index.php?g=User&m=Upyun&a=upload&local=1&url='.urlencode(C('site_url').'/'.$arr[0]['savepath'].$arr[0]['savename']));
			/*$this->redirect('Upyun/uploadReturn',array('code'=>200,'url'=>C('site_url').'/'.$arr[0]['savepath'].$arr[0]['savename']));
			var_dump (C('site_url').'/'.$arr[0]['savepath'].$arr[0]['savename']);die;*/
                             }
	}
