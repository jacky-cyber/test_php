...
class CardAction extends BaseAction{
	...
	public function get_card(){
		...
		if($card){
			header('Location:'.U('Wap/Card/vip',array('token'=>$token,'wecha_id'=>$wecha_id)));
		}
		...
		if($get_card!=false){
			Header("Location: ".U('Wap/Card/vip',array('token'=>$this->_get('token'),'wecha_id'=>$this->_get('wecha_id')))); 
		}
		if($token!=false){
			//会员卡信息
			$data=M('member_card_set')->where(array('token'=>$token))->find();
	...
	public function info(){
		...
		if($token!=false){
			...
			$contact=M('member_card_contact')->where(array('token'=>$token))->order('sort desc')->find();
			//使用说明积分说明
			$shuoming=M('member_card_exchange')->where(array('token'=>$token))->order('id desc')->find();
			//我的卡号
	...
	public function vip(){
		...
		$card=M('member_card_create')->where(array('token'=>$token,'wecha_id'=>$wecha_id))->find();
		if($card==false){
			header('Location:'.U('Wap/Card/get_card',array('token'=>$token,'wecha_id'=>$wecha_id)));
		}
		//
	   $agent = $_SERVER['HTTP_USER_AGENT']; 
		if(!strpos($agent,"MicroMessenger")) {
