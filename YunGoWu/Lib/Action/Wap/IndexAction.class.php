...
class IndexAction extends BaseAction{
  ...
  KEY `token` (`token`,`taxis`,`display`)
		...
		//dump($where);
		$info=M('Classify')->where(array('token'=>$this->_get('token'),'status'=>1,'pid'=>0))->order('sorts desc')->select();
		$info=$this->convertLinks($info);//加外链等信息

		$info2=M('Classify')->where(array('token'=>$this->_get('token'),'status'=>1,'pid'=>array('gt',0)))->order('sorts desc')->select();
	...
	public function index(){
        ...
        $new=M('classify')->where(array('pid'=>$this->_get('classid'),'token'=>$this->token))->select();
...
<script src="'.RES.'/css/bjdh/'.$this->homeInfo['bjdh'].'/bjdh'.$this->homeInfo['bjdh'].'.js" type="text/javascript"></script>');
	...
	public function lists(){
		...
		$new=M("classify")->where(array("pid"=>$l["pid"],"token"=>$this->token))->select();
		if($new)		$this->assign('info',$new);

        $ifone=M("classify")->where(array("pid"=>$this->_get("classid")))->select();
