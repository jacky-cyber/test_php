...
class ProductAction extends BaseAction{
	...
	public function cats(){
		...
		$this->assign('metaTitle','商品分类');
		        ...
		        $new=$this->product_cat_model->where(array('parentid'=>$this->_get('catid')))->select();
		if($new)$this->assign('cats',$new);
	...
	public function products(){
		...
		$products = $this->product_model->where($where)->order($order.' '.$method)->limit('6')->select();
		$this->assign('products',$products);
		$this->assign('metaTitle','商品');


         $ifone=D('product_cat')->where(array("parentid"=>$this->_get("catid")))->select();
		if($ifone)
		{
		$this->redirect(U('Product/cats',array('catid'=>$this->_get('catid'),'token'=>$this->token)));
