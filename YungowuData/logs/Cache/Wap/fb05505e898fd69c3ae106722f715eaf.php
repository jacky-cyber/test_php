...
<section class="c_list">
	...
	<?php if(is_array($cats)): $i = 0; $__LIST__ = $cats;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><dl class="leibie">
		<dt>
			<a href="<?php if($hostlist['parentid'] == 0): echo U('Product/products',array('token'=>$_GET['token'],'catid'=>$hostlist['id'],'wecha_id'=>$wecha_id,'dining'=>$isDining)); else: echo U('Product/products',array('token'=>$_GET['token'],'catid'=>$hostlist['id'],'wecha_id'=>$wecha_id)); endif; ?>"><br><img src='<?php echo ($hostlist["logourl"]); ?>' width='100' height='100'/>
				<br><?php echo ($hostlist["name"]); ?>
				<i></i>
			</a>
		</dt>
