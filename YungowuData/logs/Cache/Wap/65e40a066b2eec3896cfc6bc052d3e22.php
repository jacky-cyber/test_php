...
<section class="f_list i_list" >
		<ul class="clearfix" id="f_list" style="text-align:center;">
		<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$hostlist): $mod = ($i % 2 );++$i;?><li class="shangpin">
			<div >
				<a href="<?php echo U('Product/product',array('token'=>$_GET['token'],'id'=>$hostlist['id'],'wecha_id'=>$_GET['wecha_id']));?>">
					...
					<div style="width:100%;line-height:24px;text-align:center;"><?php echo ($hostlist["name"]); ?><p class="tui_price ts2" style="font-weight:normal;font-size:16px;color:#666;"><del>￥&nbsp;<?php echo ($hostlist["oprice"]); ?></del>&nbsp;￥<?php echo ($hostlist["price"]); ?></p></div>
					
										<div class="hot">
						<strong <?php if($hostlist["groupon"] != 1): ?>style="display:none"<?php endif; ?>>团购</strong>
					</div>
									</a><p class="tui_price ts2">
					<strong></strong>
				</p>
			</div>
			<div>
				...
				$.ajax({
					...
					success : function(RES) {
						...
						$.each(data, function(x, y) {
							_tmp_html +=    '<li  class="shangpin"><div >' +
							...
							_tmp_html += '<p class="tui_price ts2"><strong>￥' + y.price + '</strong></p></div><div>' +
