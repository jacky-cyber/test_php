...
<div class="cLineB">
  <h4>首页回复配置 </h4><a href="javascript:history.go(-1);" class="right btnGrayS vm" style="margin-top:-27px">返回</a>
    ...
    <div class="msgWrap bgfc">
     ...
     <tr>
    <th >背景音乐:</th>
     <td><input class="px"  name="bjmp3" value="<?php echo ($home["bjmp3"]); ?>"   id="suicai3"  style="width:300px"  /></td>
    </tr>
  ...
  <?php for($i=0;$i<=5;$i++) { if($i==0)$dhname='无动画'; if($i==1)$dhname='下落的枫叶'; if($i==2)$dhname='飘雪'; if($i==3)$dhname='漂雨'; if($i==4)$dhname='飘玫瑰'; if($i==5)$dhname='百叶窗'; if($home['bjdh']==$i){ $list=$list.'<option selected  value="'.$i.'">'.$dhname.'</option>'; } else { $list=$list.'<option value="'.$i.'">'.$dhname.'</option>'; } } echo $list; ?>
                   
                    </select> （暂时只支持模版12、13、14、15、17、39）</TD>
                </tr>
		...
		<tr>
  <th valign="top" > 设置页面版权:</th>
 
<td  ><textarea    class="px"  style="width:350px; height:35px"  name="copyright" ><?php echo ($home["copyright"]); ?></textarea><br />© 2001-2013 某某版权所有 </td>

</tr>

<TR>
                  <th valign="top" >  统计代码：</TH>
                  <TD><textarea    class="px"  style="width:350px; height:35px"  name="tongji" ></textarea> <br />建议使用 <a href="http://www.51.la/" >51统计</a>,不超过300字符<br />例如&lt;img src=&quot;http://shopweb.51.la/8/3/83711.asp&quot; /&gt;</TD>
                </TR>
