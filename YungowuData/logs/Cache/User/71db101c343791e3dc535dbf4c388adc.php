	...
	$(".nav-header").mouseover(function(){
        ...
        <div class="content">
          ...
          <div class="cLineB">
              ...
              <h4 class="left">微喜帖活动信息 (<?php echo ($count); ?>) 条<span class="FAQ">你本月有 <?php echo ($group["activitynum"]); ?> 次机会可免收活动创建费，已经使用了 <?php echo ($activitynum); ?> 次机会!</span></h4>
          ...
          <div class="cLine">
			...
			<a href="<?php echo U('Marrycard/add',array('token'=>$token));?>" title='新增微喜帖' class='btnGrayS vm bigbtn'>新增微喜帖</a>
...
<TH class="norightborder">操作</TH>
              ...
              <TBODY>
                ...
                <TR></TR>
                  ...
                  <TD><?php echo (date('Y-m-d',$list["startdate"])); ?><br /><?php echo (date('Y-m-d',$list["enddate"])); ?></TD>

				  <TD><?php echo (date('Y-m-d H:i',$list["partytime"])); ?></TD>

                  <TD>

				  <?php if($list['status'] == 0): ?>未开始<?php elseif($list['status'] == 2): ?>已结束<?php else: ?>已开始<?php endif; ?>

				 </TD>

                  <TD width="130"><?php echo C('site_url');?>/index.php?g=Wap&m=Marrycard&a=index&id=<?php echo ($list['id']); ?>&token=<?php echo ($token); ?></TD>

                   <TD class="norightborder">

				   <a href="<?php echo U('Marrycard/wishlist',array('id'=>$list['id']));?>">祝福</a>

				   <a href="<?php echo U('Marrycard/joinlist',array('id'=>$list['id']));?>">赴宴</a>

				   <a href="<?php echo U('Marrycard/edit',array('id'=>$list['id']));?>">编辑</a>

				   <a href="

				   <?php if($list['status'] == 1): ?>javascript:drop_confirm('你确定要结束活动吗，结束后不可再开启本活动!','<?php echo U('Marrycard/end',array('id'=>$list['id']));?>');<?php else: echo U('Marrycard/start',array('id'=>$list['id'])); endif; ?>">

				   <?php if($list['status'] == 0): ?>开始<?php elseif($list['status'] == 2): ?>打开<?php else: ?>结束<?php endif; ?>				   

				   </a>

				   <a href="<?php echo U('Marrycard/delete',array('id'=>$list['id']));?>">删除</a>
